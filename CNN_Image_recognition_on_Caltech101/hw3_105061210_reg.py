...
def batch_generator(X, y, batch_size = 256, shuffle = True, i=123):
...
reg_celoss = (celoss + 0.00001 * regularizer)
...
for epoch in range(1, 30+1):
    ...
    for batch_X, batch_y in batch:
        _, cost = sess.run([train_step, reg_celoss], feed_dict={x_image: batch_X, ys: batch_y, keep_prob: 0.5})
        batch_cost.append(cost)
    train_loss.append(np.mean(batch_cost))
    print('epoch: ', epoch, '    train loss: ', train_loss[epoch-1])
    train_acc.append(sess.run(accuracy, feed_dict={x_image: train_image, ys: train_label, keep_prob: 0.5}))
